...
- 场景可以变化，但角色身份不能变化
- 避免复杂文字、复杂背景、多人同框、强透视
- 如果用户要小红书封面感，画面要留出可放封面字的空白区域
...
【输出 JSON 结构】
{
  ...
  "imageScene": "中文图片构图描述，具体到芒狗动作、表情、物品、环境",
  "imagePrompt": "English prompt for GPT Image, locking the Mango dog character identity",
...
export function buildUserPrompt(input: UserPromptInput): string {
  ...
  if (input.primaryReference) {
    ...
    parts.push('生成图片时必须沿用基准图中的角色形象，只改变动作、道具和场景。');
  } else {
